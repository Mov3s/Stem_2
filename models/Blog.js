...
BlogSchema.statics.findChunksForBlog =  async (blogs) =>{
  ...
  try{
    ...
    for (let blog of blogs){
      const imageName = blog && blog.image ? blog.image: null;

      var eachBlogImage 
      if (imageName){

          // console.log(img)
          const blobs = await imageFilesCollecttion.find({ "filename": imageName }).toArray()
      ...
      blog.image_chunk = eachBlogImage
